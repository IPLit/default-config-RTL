SELECT
  DISTINCT(sb.start_datetime)                                                AS "Operation Date & Time",
  sa_timehrs.value                                                           AS "Est. time Hrs",
  sa_timemint.value                                                          AS "Est. time Mint",
  l.name                                                                     AS 'Operation Theatre',
  ...
  sa_assistant.value                                                         AS "Surgical Assistant",
  sa_Anaesthetist.value                                                      AS "Anaesthetist",
  sa_Scrub.value                                                             AS "Scrub Nurse",
  sa_Circulating.value                                                       AS "Circulating Nurse"
  ...
  JOIN location l ON sb.location_id = l.location_id AND l.retired IS FALSE
       WHERE cast(CONVERT_TZ(sa.date_created,'+00:00','+5:30') AS DATE) BETWEEN '#startDate#' AND '#endDate#' AND sa.voided IS FALSE AND sa.status = "SCHEDULED"
